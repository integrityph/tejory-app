...
class _SettingsPageState extends State<SettingsPage> with ChangeNotifier {
  ...
  Future<void> _saveSettings() async {
    ...
    Singleton.assetList.assetListState.setCurrency(Singleton.assetList.assetListState.currencyList.firstWhere((Currency currency){
  ...
  Future<void> _loadSettings() async {
    ...
    setState(() {
      ...
      _twoFactorAuthentication = prefs.getBool('twoFactorAuthentication') ?? false;
      ...
      Singleton.loaded!.then((val){
        ...
        for (final wallet in Singleton.assetList.assetListState.walletList) {
          walletList.add(
            ListTile(
            key: Key("${wallet.id}"),
            leading: wallet.type == WalletType.phone? Icon(Icons.phone_android) : Icon(Icons.picture_in_picture_alt_rounded),
            minTileHeight: 8,
            minVerticalPadding: 0,
            title: Text(
              wallet.name!,
        ...
        for (final currency in Singleton.assetList.assetListState.currencyList) {
          _currencies.add(
            ListTile(
          ...
          _buildCard(
            child: Column(
              ...
              children: [
                TextButton(
                  style: ButtonStyle(
                    backgroundColor: WidgetStateProperty.all(const Color.fromARGB(255, 14, 154, 219),),
                    padding: WidgetStateProperty.all(EdgeInsets.only(top: 15, bottom: 15))
                  ),
                  onPressed: (){
                          ...
                          result.then((val) async {
                            ...
                            if (val is String) {
                              ...
                              try {
                                ...
                                msgDialog(Text("Failed to link terminal. Try again"), context,
                                  actions: [ok],
                                  icon: Icon(
                                    Icons.error_outline,
                                    size: 100,
                              ...
                              msgDialog(Text("Terminal linked successfully"), context,
                              actions: [ok],
                              icon: Icon(
                  ...
                  child: Center(child:Text("Link Terminal", style: TextStyle(fontWeight: FontWeight.w700, fontSize: 18)))
                )
              ],
            ),
          ),
          ...
          _buildCard(
            child: Column(
              ...
              children: [
                TextButton(
                  style: ButtonStyle(
                    backgroundColor: WidgetStateProperty.all(const Color.fromARGB(255, 192, 0, 0),),
                    padding: WidgetStateProperty.all(EdgeInsets.only(top: 15, bottom: 15))
                  ),
                  onPressed: (){
                    showDialog<void>(
                  ...
                  child: Center(child:Text("Initialize Card", style: TextStyle(fontWeight: FontWeight.w700, fontSize: 18)))
                )
              ],
            ),
          ),
  ...
  void msgDialog(Widget? content, BuildContext _context, {List<Widget>? actions, Widget? icon}) {
    showDialog<dynamic>(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
