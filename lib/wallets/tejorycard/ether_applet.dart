...
class EtherSigningOptions implements CoinSigningOptions {
  @override
  List<int>? options;

  EtherSigningOptions({this.options});
  ...
  static EtherSigningOptions Standard() {
    return EtherSigningOptions(options: [0x00]);
  }
}

class EtherApplet implements CoinApplet {
  static const int CLA = 0xc0;
  static const int INS_SIGN_PST = 0xd1;
  static const int INS_SIGN_TX = 0xd0;
  // static const List<int> AID = [0x84, 0x6a, 0x60, 0x80, 0x00, 0x00, 0x3c, 0x00];
  ...
  @override
  Future<Map<String, Uint8List>?> signPST(
      String pst, String tx, bool getRawSignature,
  ...
  @override
  Future<Map<String, Uint8List>?> signTX(
      List<String> paths, String tx, bool getRawSignature,
      {CoinSigningOptions? signingOptions}) async {
    // ensure applet is selected
    await selectApplet();
    ...
    // add the hash
    var txHash = keccak(Uint8List.fromList(tx.codeUnits));
    // cdata.addAll(
    //     Uint8List(2)..buffer.asByteData().setUint16(0, tx.length, Endian.big));
    cdata.add(txHash.length);
    cdata.addAll(txHash);
