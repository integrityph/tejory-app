...
class FixDuplicateCPK extends Update {
  Isolate? _isolate;
  ...
  Future<bool> required() async {
    ...
    if (box.walletDBBox.count() != 0) {
      final w = box.coinBox.query().build().findFirst();
      if (box.coinBox.query(Coin_.cpk.equals(w!.cpk??"")).build().count() != 1)
      return true;
    }
    return false;
  }
  ...
  Future<void> start() async {
    ...
    _receivePort.listen((message) {
      if (message is UpdateProgress) {
        ...
        print(
          "FixDuplicateCPK: ERROR: unknown message type ${message.runtimeType}. ${message}",
        );
      }
    });
  ...
  static void worker(SendPort sendPort) {
    ...
    _receivePort.listen((message) async {
      try {
        ...
        if (box == null) {
          print("FixDuplicateCPK: DB is not initialized");
          sendPort.send(
            UpdateProgress(
              0,
              done: true,
              message: "FixDuplicateCPK: DB is not initialized",
              ex: Exception("FixDuplicateCPK: DB is not initialized"),
            ),
          );
          return;
        }

        print("FixDuplicateCPK: db initialized");
        ...
        print("FixDuplicateCPK: list prepared");

        int tableCounter = 0;
        Map<String, bool> cpkRecordMap = {};
        for (final table in tablesList) {
          final records = table.getAll();
          for (int i=0; i<records.length; i++) {
            if (cpkRecordMap.containsKey(records[i].cpk!)) {
              table.box.remove(records[i].id);
            } else {
              cpkRecordMap[records[i].cpk!] = true;
            }
          }
          ...
          print(
            "FixDuplicateCPK: Done with ${tableCounter} progress ${(progress * 100)}%",
          );
          // send message
          sendPort.send(
            UpdateProgress(
              ...
              message: "FixDuplicateCPK: Completed ${table.name}",
              ex: null,
            ),
          );
        }
        ...
        sendPort.send(
          UpdateProgress(
            ...
            message: "FixDuplicateCPK: Error. ${e.toString()}",
  ...
  void stop() {
    if (_isolate != null) {
      ...
      print("FixDuplicateCPK: Isolate stopped.");
    }
  }
