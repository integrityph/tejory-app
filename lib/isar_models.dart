// // GENERATED CODE - DO NOT MODIFY BY HAND

// import 'package:tejory/collections/balance.dart.bak';
// import 'package:tejory/collections/block.dart';
// import 'package:tejory/collections/coin.dart';
// import 'package:tejory/collections/data_version.dart';
// import 'package:tejory/collections/key.dart';
// import 'package:tejory/collections/lp.dart';
// import 'package:tejory/collections/next_key.dart';
// import 'package:tejory/collections/tx.dart';
// import 'package:tejory/collections/wallet_db.dart';

// typedef TTYpe<T> = Type Function<T>();

// class Models {
//   Models._();

//   static const balance = BalanceModel();
//   static const block = BlockModel();
//   static const coin = CoinModel();
//   static const dataVersion = DataVersionModel();
//   static const key = KeyModel();
//   static const lP = LPModel();
//   static const nextKey = NextKeyModel();
//   static const txDB = TxDBModel();
//   static const walletDB = WalletDBModel();
// }
